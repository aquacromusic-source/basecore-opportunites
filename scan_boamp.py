...
API = "https://boamp-datadila.opendatasoft.com/api/explore/v2.1/catalog/datasets/boamp/records"#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
SCAN BOAMP — genere le fichier de consultations pour basecore.fr.
Extrait les VRAIS noms de lots depuis le detail (eFORMS ou schema Boamp legacy).
Les QUANTITES ne sont PAS dans l'API (DCE) : placeholder. Sortie JSON pur.
"""
...
def extract_lots(donnees_str):
            ...
            if L:
                ...
                for e in entries:
                    if isinstance(e, dict):
                        nm = e.get("INTITULE") or e.get("DESCRIPTION") or e.get("TITRE")
                        nm = _text_of(nm)
                        if nm:
                            names.append(nm)
    ...
    for i, nm in enumerate(clean):
        lots.append({"lot": i + 1, "designation": nm, **_lot_defaults(nm)})
    return lots
...
def main():
    ...
    for seq, (idw, (rec, sec)) in enumerate(sorted(seen.items(),
        ...
        if not lots:
            lots = [{"lot": 1, "designation": (rec.get("objet") or "Prestation")[:90],
                     "format": "A4", "grammage": 135, "pages": 1, "quantite": 1000}]
            source = "generique"
        items.append({
            "id": short_id(rec, seq),
